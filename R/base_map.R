...
#'
#' Create background map from a bounding box
#'
...
#' @export
#'
#' @details A helper, or stand-alone, function to create background map based on based on raster map tiles retrieved with \code{ggmap::get_stamenmap}.  If \code{ggmap} is unavailable, the function creates a basic map using county-level polygon files.  This map is fairly crude and should be considered a placeholder.
#'
...
#' bound_box <- c(-77.393, 39.741, -75.553, 38.277)
#' x <- base_map(bound_box, zoom = 10)
#'
#' \donttest{
#' # Default zoom map with watercolor maptype.
#' y <- base_map(bound_box, maptype = 'watercolor')
#' }


base_map <- function(bbox,
                     bg_crs = 4326,
                     maptype = 'toner-lite',
                     zoom = NULL,
                     ...) {
